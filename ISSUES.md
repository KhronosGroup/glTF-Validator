...
|23|NODE_MATRIX_DEFAULT|Do not specify default transform matrix.|Information|
...
|27|NODE_EMPTY|Empty node encountered.|Information|
...
|30|SKIN_IBM_INVALID_FORMAT|Invalid IBM accessor format '`%1`'. Must be one of ('`%a`', '`%b`', '`%c`'). |Error|
