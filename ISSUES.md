...
|10|BUFFER_DATA_URI_MIME_TYPE_INVALID|Buffer's Data URI MIME-Type must be `application/octet-stream` or `application/gltf-buffer`. Got `%1` instead.|
...
|3|ACCESSOR_MIN_MISMATCH|Declared minimum value for component `%1` (`%2`) does not match actual one (`%3`).|
|4|ACCESSOR_MAX_MISMATCH|Declared maximum value for component `%1` (`%2`) does not match actual one (`%3`).|
|5|ACCESSOR_ELEMENT_OUT_OF_MIN_BOUND|Accessor element `%1` at index `%2` is less than declared minimum value `%3`.|
|6|ACCESSOR_ELEMENT_OUT_OF_MAX_BOUND|Accessor element `%1` at index `%2` is greater than declared maximum value `%3`.|
|7|ACCESSOR_NON_UNIT|Accessor element at index `%1` is not of unit length: `%2`.|
|8|ACCESSOR_INVALID_SIGN|Accessor element at index `%1` has not a proper sign value in `w` component: `%2`.|
|9|ACCESSOR_INVALID_FLOAT|Accessor element at index `%1` is NaN or Infinity.|
|10|ACCESSOR_INDEX_OOB|Indices accessor element at index `%1` has vertex index `%2` that exceeds number of available vertices `%3`.|
|11|ACCESSOR_INDEX_TRIANGLE_DEGENERATE|Indices accessor contains degenerate triangle `%1` at index `%2`.|
|12|ACCESSOR_ANIMATION_INPUT_NEGATIVE|Animation input accessor element at index `%1` is negative: `%2`.|
|13|ACCESSOR_ANIMATION_INPUT_NON_INCREASING|Animation input accessor element at index `%1` is less than or equals to previous: `%2 <= %3`.|
|14|ACCESSOR_SPARSE_INDICES_NON_INCREASING|Accessor sparse indices element at index `%1` is less than or equals to previous: `%2 <= %3`.|
|15|ACCESSOR_SPARSE_INDEX_OOB|Accessor sparse indices element at index `%1` is greater than or equal to the number of accessor elements: `%2 >= %3`.|
|16|ACCESSOR_INDECOMPOSABLE_MATRIX|Matrix element at index `%1` is not decomposable to TRS.|
|17|IMAGE_DATA_INVALID|Image data is invalid. %1|
|18|IMAGE_MIME_TYPE_INVALID|Recognized image format (`%1`) does not match declared image format (`%2`).|
|19|IMAGE_UNEXPECTED_EOS|Unexpected end of image stream.|
|20|IMAGE_UNRECOGNIZED_FORMAT|Image format has not been recognized.|
|21|IMAGE_NPOT_DIMENSIONS|Image has non-power-of-two dimensions: %1x%2.|
## GlbError
| No | Name | Message |
|:---:|------------|-------------|
